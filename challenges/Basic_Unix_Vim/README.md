...
You should be familiar with the command format by now. To open a file in Vim, execute the `vi` command with the name of the file. Try to open the file `text_file.txt` with Vim.

```bash
[userid@login1.frontier Basic_Unix_Vim]$ vi text_file.txt
```
